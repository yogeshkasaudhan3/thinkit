 ...
 * generic Node host) that serves the Backend API and the Admin Panel from
 * the SAME origin — required because the admin panel's own code calls the
 * API via relative "/api/..." paths (see app.ts).
 *
 ...
 *     admin-panel-dist/                                  (admin panel static files)
...
async function main() {
  ...
  // 3. Copy the admin panel's static output next to the backend bundle.
